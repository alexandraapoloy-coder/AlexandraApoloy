...
<style>
body{font-family:Segoe UI;margin:0;height:100vh;display:flex;justify-content:center;align-items:center;
background:linear-gradient(135deg,#667eea,#764ba2);color:white;text-align:center;}
.container{background:rgba(255,255,255,0.15);padding:60px;border-radius:20px;backdrop-filter:blur(10px);width:500px;}
button{padding:12px 25px;border:none;border-radius:8px;background:#00e1ff;font-weight:bold;cursor:pointer;}
</style>
</head>
<body>
<div class="container">
...
<style>
body{font-family:Segoe UI;background:linear-gradient(135deg,#667eea,#764ba2);color:white;text-align:center;padding:40px;}
...
.container{background:rgba(255,255,255,0.15);padding:40px;border-radius:20px;backdrop-filter:blur(10px);width:400px;text-align:center;}
input{width:100%;padding:10px;margin:8px 0;border:none;border-radius:6px;}
button{width:100%;padding:12px;margin-top:10px;border:none;border-radius:8px;background:#00e1ff;font-weight:bold;cursor:pointer;}
</style>
</head>
<body>
<div class="container">
...
fetch("/student",{method:"POST",headers:{"Content-Type":"application/json"},
body:JSON.stringify({name:name,grade:grade,section:section})})
.then(res=>res.json()).then(data=>{alert("Student Saved")})
}
function dashboard(){window.location="/dashboard"}
</script>
</body>
</html>
""")
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<style>
body{font-family:Segoe UI;background:linear-gradient(135deg,#667eea,#764ba2);padding:40px;color:white;text-align:center;}
.section-buttons button{margin:5px;padding:8px 15px;border:none;border-radius:6px;background:#00e1ff;font-weight:bold;cursor:pointer;}
table{width:80%;margin:auto;border-collapse:collapse;background:rgba(255,255,255,0.2);}
th,td{padding:12px;border:1px solid white;}
...
fetch("/edit/"+index,{method:"PUT",headers:{"Content-Type":"application/json"},
body:JSON.stringify({name:name,grade:grade,section:section})})
.then(res=>res.json()).then(data=>{location.reload()})
}
...
btnContainer.innerHTML='<button onclick="filterSection(`ALL`)">All</button>'
sections.forEach(sec=>{btnContainer.innerHTML+=`<button onclick="filterSection('${sec}')">${sec}</button>`})
}

function filterSection(section){
let filtered=section==="ALL"?students:students.filter(s=>s.section===section)
display(filtered)
createChart(filtered)
}
